...
struct DATriePrivate {
    ...
    struct block { // a block w/ 256 elements
        ...
        friend std::ostream& operator<<(std::ostream& out, const block& b)
        {
            ...
            && marshall(out, b.next)
            && marshall(out, b.num)
            && marshall(out, b.reject)
            && marshall(out, b.trial)
            && marshall(out, b.ehead);
            return out;
        }
    };
    ...
    DATriePrivate(const DATriePrivate& other) :
      ...
      , m_tail(other.m_tail)
      , m_tail0(other.m_tail0)
      , m_block(other.m_block)
      , m_ninfo(other.m_ninfo)
      , m_bheadF(other.m_bheadF)
      , m_bheadC(other.m_bheadC)
      , m_bheadO(other.m_bheadO)
    ...
    void suffix(std::string& key, size_t len, npos_t pos)
    {
        ...
        if (const int offset = pos.offset) {
            ...
            std::copy(&m_tail[static_cast <size_t> (offset) - len_tail],
                      &m_tail[static_cast <size_t> (offset)],
                      key.begin() + len);
        }
        while (len--) {
    ...
    void update (const char* key, npos_t& npos, size_t& pos, size_t len, const updater_type &callback, T cf) {
        ...
        if (offset >= sizeof(int32_t)) { // go to m_tail
            ...
            for (auto i = offset; i <= moved; i += 1 + sizeof (value_type)) {
                if (m_tail0.capacity() == m_tail0.size()) {
                    auto quota = m_tail0.capacity() + (m_tail0.size() >= MAX_ALLOC_SIZE ? MAX_ALLOC_SIZE : m_tail0.size());
        ...
        if (m_tail.capacity() < m_tail.size() + needed) {
            auto quota = m_tail.capacity() + ((needed > m_tail.size() || needed > MAX_ALLOC_SIZE) ? needed :
    ...
    int32_t _find (const char* key, npos_t& npos, size_t& pos, const size_t len) const {
        ...
        if (! offset) { // node on trie
            for (const uchar* const key_ = reinterpret_cast <const uchar*> (key);
                ...
                size_t to = static_cast <size_t> (m_array[from].base); to ^= key_[pos];
    ...
    void _push_enode (const int e) {
        ...
        if (b.reject < m_reject[b.num]) b.reject = m_reject[b.num];
...
template<typename T>
DATrie<T>::DATrie() :
    d(new DATriePrivate<value_type>())
{
}
...
template<typename T>
void DATrie<T>::foreach (const std::string& key, callback_type func, position_type _pos)
{
    size_t pos = 0;
    typename DATriePrivate<value_type>::npos_t from(_pos);
    if (d->_find(key.c_str(), from, pos, key.size()) == NO_PATH) {
        return;
    }
...
template<typename T>
void DATrie<T>::foreach (callback_type func)
{
    d->foreach(func);
}
...
template<typename T>
void DATrie<T>::dump(std::vector<std::tuple< typename DATrie<T>::value_type , size_t,  typename DATrie<T>::position_type>>& data)
{
    data.resize(size());
    d->dump(data.data(), data.size());
}
...
template<typename T>
bool DATrie<T>::erase(const std::string& key, position_type from)
{
    return d->erase(key.c_str(), key.size(), typename DATriePrivate<T>::npos_t(from)) == 0;
}
...
template<typename T>
typename DATrie<T>::value_type DATrie<T>::exactMatchSearch(const std::string& key)
{
...
typename DATrie<T>::value_type DATrie<T>::exactMatchSearch(const char* key, size_t len)
{
    ...
    result = d->_find(key, npos, pos, len);
    if (result == NO_PATH) {
        result = NO_VALUE;
    }
    return result_value;
...
template<typename T>
typename DATrie<T>::value_type DATrie<T>::traverse(const std::string& key, position_type& from, size_t& pos)
{
    typename DATriePrivate<T>::npos_t npos(from);
    auto result = d->traverse(key.c_str(), npos, pos, key.size());
    from = npos.toInt();
    return result;
}
