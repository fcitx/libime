#ifndef LIBIME_DYNAMICTRIE_IMPL_H
#define LIBIME_DYNAMICTRIE_IMPL_H
...
{
    template<typename V>
    typename DynamicTrie<V>::iterator DynamicTrie<V>::end()
    {
        return iterator();
    }
    template<typename V>
    typename DynamicTrie<V>::iterator DynamicTrie<V>::match(const uint8_t* key, std::size_t len)
    {
        return iterator();
    }
...
#endif // LIBIME_DYNAMICTRIE_IMPL_H
