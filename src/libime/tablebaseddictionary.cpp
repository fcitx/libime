...
static const char* strConst[2][STR_LAST] = {
    ...
    { "KeyCode=", "Length=", "InvalidChar=", "Pinyin=", "PinyinLength=" , "[Data]", "[Rule]", "Prompt=", "ConstructPhrase="}
};
...
struct TableBasedDictionaryPrivate
{
    ...
    TableBasedDictionaryPrivate(const TableBasedDictionaryPrivate& other) :
      ...
      , ignoreChars(other.ignoreChars)
      , pinyinKey(other.pinyinKey)
      , promptKey(other.promptKey)
      , phraseKey(other.phraseKey)
      , codeLength(other.codeLength)
      , phraseTrie(other.phraseTrie)
...
void updateReverseLookupEntry(DATrie<int32_t>& trie, const boost::string_ref& key, const boost::string_ref& value)
{
    auto reverseEntry = value.to_string() + '\x01';
    bool insert = true;
    trie.foreach(reverseEntry, [&trie, &key, &value, &insert, &reverseEntry](int32_t, size_t len, DATrie<int32_t>::position_type pos) {
        auto oldKeyLen = len;
...
TableBasedDictionary::TableBasedDictionary() :
    d(new TableBasedDictionaryPrivate)
{
    d->reset();
}
...
TableBasedDictionary::TableBasedDictionary(const libime::TableBasedDictionary& other) :
    d(new TableBasedDictionaryPrivate(*other.d))
{
}
    ...
    while(!in.eof()) {
        ...
        switch(phase) {
            ...
            case BuildPhase::PhaseData: {
                ...
                auto key = boost::string_ref(buf).substr(0, spacePos);
                auto value = boost::string_ref(buf).substr(wordPos);
...
void TableBasedDictionary::dump(std::ostream& out)
{
    ...
    std::string buf;
    if (d->promptKey) {
        d->promptTrie.foreach([this, &buf, &out](int32_t, size_t _len, DATrie<int32_t>::position_type pos) {
            d->promptTrie.suffix(buf, _len, pos);
            auto sep = buf.find('\x01');
            boost::string_ref ref(buf);
            out << d->promptKey << ref.substr(0, sep) << " " << ref.substr(sep + 1) << std::endl;
            return true;
        });
    }
    if (d->phraseKey) {
        d->singleCharConstTrie.foreach([this, &buf, &out](int32_t, size_t _len, DATrie<int32_t>::position_type pos) {
    ...
    d->phraseTrie.foreach([this, &buf, &out](int32_t , size_t _len, DATrie<int32_t>::position_type pos) {
    ...
    d->pinyinPhraseTrie.foreach([this, &buf, &out](int32_t , size_t _len, DATrie<int32_t>::position_type pos) {
...
bool TableBasedDictionary::insert(const std::string& key, const std::string& value, PhraseFlag flag, bool verifyWithRule)
{
...
bool TableBasedDictionary::generate(const std::string& value, std::string& key)
{
    ...
    for (const auto& rule : d->rules) {
        ...
        for (const auto& ruleEntry : rule.entries) {
            ...
            d->singleCharConstTrie.foreach(s, [this, &len, &entry](int32_t, size_t _len, DATrie<int32_t>::position_type pos) {
                len = _len;
