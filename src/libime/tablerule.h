...
namespace libime
{
enum class TableRuleEntryFlag : std::uint32_t {
    FromFront,
    FromBack
};

enum class TableRuleFlag : std::uint32_t {
    LengthLongerThan,
    LengthEqual
};
...
struct TableRule {
    ...
    TableRule(const std::string& ruleString, unsigned int maxLength)
    {
        ...
        auto afterEqualSign = boost::string_ref(ruleString).substr(equalSignPos + 1);
        ...
        auto beforeEqualSign = boost::string_ref(ruleString).substr(0, equalSignPos);
    ...
    TableRule(TableRuleFlag _flag = TableRuleFlag::LengthEqual, int _phraseLength = 0, std::vector<TableRuleEntry> _entries = decltype(entries)()) :
        flag(_flag),
    ...
    TableRule(TableRule&& other) noexcept : flag(other.flag), phraseLength(other.phraseLength), entries(std::move(other.entries)) {
    }
    ...
    std::string toString() const {
        ...
        for (const auto &entry : entries) {
            ...
            ss << ((entry.flag == TableRuleEntryFlag::FromFront) ? 'p' : 'n') << static_cast<char>('0' + entry.character) << static_cast<char>('0' + entry.encodingIndex);
        }
        return ss.str();
    }
